...
void picture_line(picture_t *pic, pixel start, pixel end, int on) {
    ...
    if (start.y == end.y){
            for (tmp=start.x; tmp<=start.y; tmp++) picture_setPixel(pic, tmp, start.y, on);
            return;
    }
    if (start.x == end.x){
            for (tmp=start.y ; tmp<=end.y ; tmp++) picture_setPixel(pic, start.x, tmp, on);
            return;
    }
    ...
    {
            inv=1;
            tmp=start.y; start.y=start.x; start.x=tmp;
            tmp=end.y;   end.y=end.x;     end.x=tmp;
    }
    else
            inv=0;
    ...
    if ((end.x-start.x)==0)
            ...
            steigung=((float)(end.y-start.y))/((float)(end.x-start.x));
...
void picture_circle(picture_t *pic, pixel center, unsigned int radius, unsigned int on) {
    ...
    for(alpha = 0.001 ; alpha < M_PI/2 - (0.0) ; alpha += 0.002) {
        ...
        {
            picture_setPixel(pic, center.x+(unsigned int)a, center.y+(unsigned int)h, on);
        }
        ...
        if(!(   (int)center.x-(int)a < 0
             || (int)center.y-(int)h < 0))
        {
            picture_setPixel(pic, center.x-(unsigned int)a, center.y-(unsigned int)h, on);
        }
